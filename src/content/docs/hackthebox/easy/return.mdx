...
<Findings>
  ...
  <Finding port="80/tcp">IIS 10.0, serving the HTB Printer Admin Panel</Finding>
  <Finding port="88/tcp">Kerberos</Finding>
  <Finding port="389/tcp">LDAP for `return.local`</Finding>
  <Finding port="445/tcp">SMB, signing required</Finding>
  <Finding port="3268/tcp">Global Catalog</Finding>
  <Finding port="5985/tcp">WinRM, the remote shell once I have credentials</Finding>
</Findings>
...
<AttackPath
  ...
  nodes={[
    { kind: 'Web', name: 'Printer Panel', detail: [ { k: 'port', v: '<code>80/tcp</code> IIS, unauthenticated' }, { k: 'holds', v: 'a directory bind for <code>svc-printer</code>' } ] },
    { kind: 'User', name: 'svc-printer', edge: 'leaks via <b>LDAP pass-back</b>', detail: [ { k: 'via', v: 'cleartext bind caught on <code>nc -lvnp 389</code>' }, { k: 'shell', v: '<code>evil-winrm</code>, user.txt' } ] },
    { kind: 'Group', name: 'Server Operators', edge: 'is <b>member of</b>', detail: [ { k: 'note', v: 'builtin group, can reconfigure services' } ] },
