...
<Toggle label="Is the host reachable? <code>ping</code>">
...
<Toggle label="Service and version scan <code>nmap -sC -sV</code>">
...
An aggressive scan (`nmap -A`) confirmed Linux 5.x and added nothing actionable.

<Toggle label="Aggressive scan, OS detection <code>nmap -A</code>">
...
A `405 Method Not Allowed` is a hint, not a wall: the route exists, it just wants a different verb. So I sent `/search` a `POST` through Burp to see how it expects its query.

![Burp Suite request to /search showing the engine and query parameters](/images/hackthebox/easy/busqueda/busqueda-2.png)
*The form posts `engine` and `query`. The footer also reveals the engine: Searchor 2.4.0.*
...
*The advisory: user input reaches `eval()`, so a crafted query runs arbitrary Python.*
...
<Toggle label="Reading remote credentials <code>.git/config</code>">
...
<Toggle label="Where can svc act as root? <code>sudo -l</code>">
...
`docker-inspect` takes a format string and a container name, so I can read a container's full config, including its environment variables. I pulled Gitea's config and piped it through `jq`.
...
The `administrator` scripts repo in Gitea shows the source of `system-checkup.py`. The `full-checkup` action is the weak point: it runs a helper script by a relative path.
...
That single `./` is the whole vulnerability: the script calls `./full-checkup.sh`, which resolves against the current working directory, and I control that directory when I invoke it. The sudoers `secure_path` only sanitizes `PATH` lookups, not a path the script builds itself.
...
# On Kali, penelope catches and upgrades the shell:
┌──(Idan@Kali)-[~/Busqueda]
└─> penelope -p 4444
